...
interface IDataTableProps {
    data: UrlData[]; // Remove map declaration
}

const DataTable: React.FunctionComponent<IDataTableProps> = ({ data = [] }) => { // Default empty array if data is undefined
    console.log("data in dataTable is", data);
    const copyToClipboard = (url: string) => async () => {
try{
            ...
            return (
                <tr key={item._id} className="border-b text-white bg-gray-600 hover:bg-white hover:text-gray-800">
                    <td className="px-6 py-3">
                        <Link to={`${serverUrl}/shortUrl/${item.fullUrl}`} target="blank" rel="noreferer noopener">
                            {item.fullUrl}
                        </Link>
                    </td>
    ...
    return (
        <div className="container mx-auto pt-2 pb-10">
            <div className="relative overflow-x-auto shadow-sm sm:rounded-lg">
                <table className="w-full table-fixed text-sm rtl:text-right text-gray-500">
