...
const shortUrl_1 = require("../models/shortUrl"); // Ensure the correct path and file name
...
const getAllUrl = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (shortUrls.length === 0) {
            ...
            res.status(200).send(shortUrls); // Return the found URLs
        }
    }
    catch (error) {
        res.status(500).send({ message: "Something went wrong" });
...
const getUrl = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (!shortUrl) {
            ...
            shortUrl.Clicks++;
            shortUrl.save(); // Make sure to await saving the update
            res.redirect(`${shortUrl.fullUrl}`); // Use backticks for string interpolation
        }
    }
    catch (error) {
        res.status(500).send({ message: "Something went wrong" });
...
const deleteUrl = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const shortUrl = yield shortUrl_1.urlModel.findOne({ shortUrl: req.params.id });
        if (shortUrl) {
            yield shortUrl_1.urlModel.findByIdAndDelete({ _id: req.params.id }); // Delete the URL
